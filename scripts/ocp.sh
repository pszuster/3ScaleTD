...
ROUTER_POD_ID=$(oc get pod -l router=router -n default -o json | jq --raw-output .items[0].metadata.name)
oc delete pod $ROUTER_POD_ID --grace-period=0 --force=true -n default
...
oc new-app --template=sso72-x509-https --param HOSTNAME_HTTP=sso-unsecured.$SUFFIX --param HOSTNAME_HTTPS=sso.$SUFFIX --param SSO_ADMIN_USERNAME=admin --param SSO_ADMIN_PASSWORD=password --param SSO_SERVICE_USERNAME=admin --param SSO_SERVICE_PASSWORD=password --param SSO_REALM=3scaleRealm
